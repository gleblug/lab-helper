...
{
    std::string response;
    while (response.length() <= to_find.length())
        response = query(comm);
    
//     std::cout << response << std::endl;

    ind_t value_ind = response.find(to_find) + to_find.length() + 1;
    response.erase(0, value_ind);
...
//     std::cout << response << std::endl;
    double value = strtod(response);
    return value;
}

void Oscilloscope::set_memory_size(MSIZ size)
{
    std::string size_str;
    ...
    command("MSIZ " + size_str);
}

void Oscilloscope::write_waveform(xt::xarray<double> &waveform)
{
    ...
    std::vector<std::size_t> shape = {bytes_count};
    waveform = xt::adapt(wf, bytes_count, xt::no_ownership(), shape);
    waveform = waveform * vdiv - voffset;
